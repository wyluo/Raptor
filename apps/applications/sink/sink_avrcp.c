...
static void setPlayStatus (uint16 index, SinkAvrcp_play_status newStatus)
{
    ...
    //add by lwy 20200615
#if 1
    if((newStatus==2)&&(My_theSink.DA7217_Vol==0))
    {

        MessageSendLater(&theSink.task,EventUsrGaiaUser6,0,0);//disable AMP
        //MessageSendLater(&theSink.task,EventUsrGaiaUser6,0,1200);//disable AMP
        DA7217_Enter_Standbymode();
    }
    else if((newStatus==1)&&(My_theSink.DA7217_Vol==0))
    {
        MessageSendLater(&theSink.task,EventUsrGaiaUser2,0,0);//enable AMP
        //MessageSendLater(&theSink.task,EventUsrGaiaUser2,0,1700);//enable AMP//?        DA7217_Enter_Standbymode();
    }
#endif
    //endof
